...
from cnn.training import build_cnn_model, cnn_recipes, scale_cnn_full_windows
...
if is_uploaded_mode:
    overview_tab, aleyna_tab, gizem_tab, cnn_tab, lstm_tab, compare_tab, live_tab = st.tabs(
        ["Veri İnceleme", "Sinyal İşleme Filtreleme", "Özellik Çıkarımı (Feature Engineering)", "CNN Eğitimi", "LSTM Eğitimi", "CNN LSTM Karşılaştırma", "Canlı Tahmin"]
    )
else:
    overview_tab, simay_tab, aleyna_tab, gizem_tab, cnn_tab, lstm_tab, compare_tab, live_tab = st.tabs(
        ["Veri İnceleme", "Fiziksel Modelleme ve Donanım", "Sinyal İşleme Filtreleme", "Özellik Çıkarımı (Feature Engineering)", "CNN Eğitimi", "LSTM Eğitimi", "CNN LSTM Karşılaştırma", "Canlı Tahmin"]
    )
...
if not is_uploaded_mode:
    with simay_tab:
        ...
        try:
            ...
            simay_rows = st.slider("Gosterilecek satir sayisi", 5, min(len(simay_df), 100), 20, key="simay_rows")
            simay_max_start = max(len(simay_df) - simay_rows, 0)
            simay_start = st.slider("Baslangic satiri", 0, simay_max_start, 0, key="simay_start")
...
with gizem_tab:
    ...
    try:
        ...
        if len(signal_for_plot) >= 3:
            ...
            valley_prominence_threshold = 0.15
            valid_valleys = []
            for idx in valley_candidates:
                prominence = min(signal_for_plot[idx - 1], signal_for_plot[idx + 1]) - signal_for_plot[idx]
                if prominence >= valley_prominence_threshold:
                    valid_valleys.append(int(idx))

            min_valley_distance_time = 30.0
            selected_valleys = []
            for idx in valid_valleys:
                if not selected_valleys:
                    selected_valleys.append(idx)
                    continue
                if abs(x_for_plot[idx] - x_for_plot[selected_valleys[-1]]) >= min_valley_distance_time:
                    selected_valleys.append(idx)
                elif signal_for_plot[idx] < signal_for_plot[selected_valleys[-1]]:
